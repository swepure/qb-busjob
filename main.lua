            ...
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        -- Open the doors
                        SetVehicleDoorOpen(veh, 0, false, false) -- Open front left door
                        SetVehicleDoorOpen(veh, 1, false, false) -- Open front right door
                        ...
                        -- Close the doors after the NPC exits
...
RegisterNetEvent('qb-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            FreezeEntityPosition(NpcData.Npc, false)  -- Make sure the NPC is unfrozen and can move
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                -- Move NPC close to the door
                                local frontLeftDoor = GetWorldPositionOfEntityBone(veh, GetEntityBoneIndexByName(veh, "door_dside_f"))
                                TaskGoStraightToCoord(NpcData.Npc, frontLeftDoor.x, frontLeftDoor.y, frontLeftDoor.z, 1.0, 20000, 0.0, 0.0)

                                -- Small wait to ensure NPC is properly aligned
                                Wait(2000)

                                -- Attempt to have the NPC enter the vehicle
                                TaskEnterVehicle(NpcData.Npc, veh, 20000, 0, 1.0, 1, 0)

                                -- Wait for the NPC to enter or timeout
                                local timeout = 10000 -- 10 seconds
                                local timer = 0
                                while not IsPedInVehicle(NpcData.Npc, veh, false) and timer < timeout do
                                    Wait(100)
                                    timer = timer + 100
                                end

                                if not IsPedInVehicle(NpcData.Npc, veh, false) then
                                    -- Fallback: Warp NPC into the vehicle if all else fails
                                    TaskWarpPedIntoVehicle(NpcData.Npc, veh, 0)
                                    QBCore.Functions.Notify(Lang:t('info.npc_warped_in'), 'primary')
                                end

                                -- Close the doors after the NPC is inside
                                SetVehicleDoorShut(veh, 0, false)
                                SetVehicleDoorShut(veh, 1, false)
